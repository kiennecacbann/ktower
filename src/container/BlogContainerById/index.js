  ...
  return (
    <div className='pt-[50px]'>
      <div className='items-center justify-center flex flex-col'>
        ...
        <h2 className='text-[46px] font-extrabold uppercase'>All BLOGS</h2>
      </div>
      <div className='grid lg:grid-cols-4 md:grid-cols-2 gap-5 py-10 px-10'>
        {data?.length > 0 && data?.map(item => (
          <Link to={`/blog/${item._id}`} className='block max-w-[450px] w-full max-h-[470px] h-full cursor-pointer'
                key={item._id}>
            <div className='flex'>
      ...
      {data?.length === 0 && (
        <div className={'text-center'}><p className={'text-red-500 text-center'}>Hiện tại chưa có bài viết nào</p>
        </div>)}
    </div>
  )
}

export default BlogContainerById