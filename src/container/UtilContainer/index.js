  ...
  return (
    <div className='pt-[50px]'>
      <div className='items-center justify-center flex flex-col'>
        ...
        <h2 className='text-[46px] font-extrabold'>RECENT UTILS</h2>
      </div>
      <div className='grid lg:grid-cols-4 md:grid-cols-2 gap-5 py-10 px-10'>
        {data?.slice(0, 4).map(item => (
          <Link to={`/utils/categories/${item._id}`}
                className='block max-w-[450px] w-full max-h-[470px] h-full cursor-pointer'
                key={item._id}>
            <div className='flex'>
            ...
            <div>
              <div className='pb-5 pt-2'>
                ...
                <p className={'my-2 text-xs text-gray-600 line-clamp-2 min-h-[50px]'}>
                  {item?.description}
                </p>
                <p>By Admin</p>
              </div>
